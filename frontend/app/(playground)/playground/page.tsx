...
import { Play, Plus, X, ThumbsUp, ThumbsDown } from "lucide-react";
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <input
                ...
                max={2}
                step={0.1}
                className="w-20 px-2 py-1 rounded bg-background/50 border border-border/50 text-sm"
              />
            </div>
          </div>
        </div>
            ...
            <GlassCard key={i}>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-1">
                  <button className="p-1 rounded hover:bg-green-500/10 hover:text-green-500 transition-colors">
              ...
              <div className="mt-4 pt-3 border-t border-border/50 grid grid-cols-3 gap-2 text-center text-sm">
                <div>
                  <p className="font-medium">{r.latency_ms}ms</p>
                  <p className="text-xs text-muted-foreground">Latency</p>
                </div>
                <div>
                  <p className="font-medium">${r.cost_usd?.toFixed(4)}</p>
                  <p className="text-xs text-muted-foreground">Cost</p>
                </div>
                <div>
                  <p className="font-medium">{r.tokens}</p>
